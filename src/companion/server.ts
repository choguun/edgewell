...
export function buildRouter({ ew, secret, logger = null }) {
  const r = new Router();
...
export function startCompanion({ ew, secret = null, port = 8787, host = "127.0.0.1", logger = null } = {}) {
  const router = buildRouter({ ew, secret, logger });
