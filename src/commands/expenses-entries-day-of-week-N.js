// `edgewell expenses-entries-day-of-week-N <0..6> <N>`
// lists the Nth occurrence of a specific day-of-week.

import { c } from "../cli.js";

export async function expensesEntriesDayOfWeekNCommand(args, ew) {
  const dow = Number(args[0]);
  const N = Number(args[1]);
  if (!Number.isFinite(dow) || dow < 0 || dow > 6 || !Number.isFinite(N) || N < 1) {
    console.error("usage: edgewell expenses-entries-day-of-week-N <0..6> <N>");
    process.exit(2);
  }
